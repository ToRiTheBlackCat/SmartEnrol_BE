...
namespace SmartEnrol.Infrastructure
{
    public class PostRetrieval
    {
        ...
        public async Task<string> ChatWithHistory(string userInput, string data, List<string> history)
        {
            ...
            if (data.Contains("None"))
            {
                var prompt = $"Your name is SmartEnrol - You are an AI consultant assistant specialized in giving advice abouts admission method of universities, major in VietNam. If the question is outside your scope, still try to answer as helpfully as possible. However, if the user asks a greeting question such as 'hello', 'hi', 'xin chào', etc., always respond with the predefined reply you have been trained with: Xin chào tôi tên là SmartEnrol - Một trợ lý ảo được tạo ra để hỗ trợ việc tư vấn tuyển sinh đại học. Tôi có thể giúp gì cho bạn không?. Here is the question: {userInput}";
